...
static uint8_t sd_send_op_cond(sdcard_t *card)
{
    ...
    do
    {
        ...
        if (card->voltage & MMC_VDD_27_36)
            ...
            cmd.cmdarg = 0;
        if (card->version == MMC_VERSION_SD_2)
            cmd.cmdarg |= MMC_OCR_HCS;
        cmd.resptype = MMC_RESP_R3;
        if (!sdc_transfer(card->sdc_base, &cmd, NULL) || (cmd.response[0] & MMC_OCR_BUSY))
            break;
    } while (retries--);
    ...
    if (card->version != MMC_VERSION_SD_2)
        card->version = MMC_VERSION_SD_1_0;
...
//    card->cid[3] = cmd.response[3];
    ...
    if (card->high_capacity)
    {
        ...
        {
            card->blk_cnt = card->extcsd[212] << 0 | card->extcsd[212 + 1] << 8 | card->extcsd[212 + 2] << 16
...
uint64_t sdcard_read(sdcard_t *card, uint8_t *buf, uint64_t blkno, uint64_t blkcnt)
{
    ...
    while (blks > 0)
    {
        ...
        if (mmc_read_blocks(card, buf, blkno, cnt) != cnt)
            return 0;
        blks -= cnt;
        blkno += cnt;
...
uint64_t sdcard_write(sdcard_t *card, uint8_t *buf, uint64_t blkno, uint64_t blkcnt)
{
    ...
    {
        cnt = (blks > 127) ? 127 : blks;
        if (mmc_write_blocks(card, buf, blkno, cnt) != cnt)
            return 0;
        blks -= cnt;
        blkno += cnt;
