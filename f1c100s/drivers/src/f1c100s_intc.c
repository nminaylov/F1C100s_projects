/********************************************//**
 * @file f1c100s_intc.c
 * @brief F1C100s Interrupt contoroller driver (INTC)
 * @author n.minaylov
 ***********************************************/
...
{
    uint32_t reg = F1C100S_INTC_BASE+INTC_PRIORITY0+irq/16*4;
...
void intc_init(void)
{
    write32(F1C100S_INTC_BASE+INTC_ENABLE0,0); // Disable all interrupts
    write32(F1C100S_INTC_BASE+INTC_ENABLE1,0);
    memset(irq_handlers, 0, sizeof(irq_handlers)); // Clear handlers table
    write32(F1C100S_INTC_BASE+INTC_BASE_ADDR,0); // Set offset to 0
}

// Global IRQ handler
void irq_handler_global(void)
{
    uint32_t irq_src = read32(F1C100S_INTC_BASE) >> 2;
